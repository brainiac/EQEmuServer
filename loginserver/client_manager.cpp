...
static const OpcodeValueList* GetTOBLoginOpcodes()
{
	static OpcodeValueList opcodes = {
#define OPCODE(name, value) { name, value },
		#include "common/patches/login_tob_opcodes.h"
#undef OPCODE
	};
...
ClientManager::ClientManager()
{
	...
	//
	// TOB Login Protocol Support
	//

	int tob_port = server.config.GetVariableInt("client_configuration", "tob_port", 15900);
	std::string tob_opcodes_file = server.config.GetVariableString("client_configuration",
		"tob_opcodes", "login_opcodes_steam_latest.conf");

	EQStreamManagerInterfaceOptions tob_opts(tob_port, false, false);
	m_tob_stream = new EQ::Net::EQStreamManager(tob_opts);

	std::string tob_opcodes_path = PathManager::Instance()->FindFilePath(PathLocation::Opcodes, tob_opcodes_file);
	m_tob_ops = new OpcodeManager("login_tob", GetTOBLoginOpcodes());

	if (!m_tob_ops->LoadOpcodes(tob_opcodes_path))
	{
		LogError("ClientManager fatal error: couldn't load opcodes for TOB file [{}]", tob_opcodes_file);
		run_server = false;
	}
